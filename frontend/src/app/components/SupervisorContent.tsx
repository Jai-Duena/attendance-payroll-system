...
export default function SupervisorContent() {
  ...
  const [drillDown, setDrillDown]         = useState<{ label: string; employeeId?: number; status?: string } | null>(null);
  ...
  const fetchRequests = useCallback(() => {
    setReqLoading(true);
    requestsApi
      .list({ employee: filterEmployee || undefined })
      .then((data) => {
        setRequests(data);
  ...
  useEffect(() => {
    ...
    requestsApi
      .summary()
      .then((data) => setEmpSummary(data.employees))
      .catch(() => {})
      .finally(() => setSumLoading(false));
  }, []);

  const applyFilter = () => setFilterEmployee(draftEmployee);
  const clearFilter = () => {
    setDraftEmployee('');
    setFilterEmployee('');
  };
  ...
  const openDrillDown = useCallback(
    (label: string, filters: { employeeId?: number; status?: string }) => {
      setDrillDown({ label, ...filters });
      setDrillVisible(REQUESTS_PAGE);
      setDrillLoading(true);
      requestsApi
        .list({ employee_id: filters.employeeId, status: filters.status })
  ...
  const refreshDrill = useCallback(() => {
    if (!drillDown) return;
    requestsApi
      .list({ employee_id: drillDown.employeeId, status: drillDown.status })
      .then((data) => setDrillRequests(data))
      .catch(() => {});
  }, [drillDown]);
...
<div class="sigs">
  ...
  // ── Helpers ──────────────────────────────────────────
  const getDisplayStatus = (req: LeaveRequest) => (req as any).sup_status || req.status || 'Pending';
  ...
  const isUrgent = (req: LeaveRequest) => {
    ...
    const d = (req as any).effective_date;
    if (!d) return false;
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              <div className={`${activeView === 'requests' ? 'bg-blue-500' : 'bg-red-500'} text-white rounded-lg p-2`}>
                {activeView === 'requests' ? <Users size={24} /> : <BarChart3 size={24} />}
              </div>
              <h2 className="text-xl md:text-2xl font-bold text-gray-800">
                {activeView === 'requests' ? 'Department Requests' : 'Employee Request Summary'}
              </h2>
            </div>
          ...
          {activeView === 'requests' && (
            <>
              ...
              <div className="flex flex-wrap gap-3 mb-5 p-3 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {filterEmployee && (
                  <button onClick={clearFilter} className="px-4 py-2 bg-gray-200 text-gray-700 text-sm rounded-lg hover:bg-gray-300 transition-colors">
                    Clear
                  </button>
                )}
              </div>
          ...
          {activeView === 'summary' && (
            <div>
              <h3 className="text-lg font-bold text-gray-700 mb-4 flex items-center space-x-2">
                ...
                <span>Employee Request Summary</span>
              </h3>
                      ...
                      <div key={i} className="p-4 bg-blue-50 rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm mb-2">
                          <div className="flex justify-between">
                            ...
                            <button onClick={() => openDrillDown(`${emp.employee_name} -- All`, { employeeId: emp.employee_id })} className="font-semibold text-blue-600 hover:text-blue-800 hover:underline cursor-pointer">{emp.total_requests}</button>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Pending:</span>
                            <button onClick={() => openDrillDown(`${emp.employee_name} -- Pending`, { employeeId: emp.employee_id, status: 'Pending' })} className="font-semibold text-yellow-600 hover:text-yellow-800 hover:underline cursor-pointer">{emp.pending}</button>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Approved:</span>
                            <button onClick={() => openDrillDown(`${emp.employee_name} -- Approved`, { employeeId: emp.employee_id, status: 'Approved' })} className="font-semibold text-green-600 hover:text-green-800 hover:underline cursor-pointer">{emp.approved}</button>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Rejected:</span>
                            <button onClick={() => openDrillDown(`${emp.employee_name} -- Rejected`, { employeeId: emp.employee_id, status: 'Rejected' })} className="font-semibold text-red-600 hover:text-red-800 hover:underline cursor-pointer">{emp.rejected}</button>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Cancelled:</span>
                            <button onClick={() => openDrillDown(`${emp.employee_name} -- Cancelled`, { employeeId: emp.employee_id, status: 'Cancelled' })} className="font-semibold text-gray-500 hover:text-gray-700 hover:underline cursor-pointer">{emp.cancelled}</button>
                          </div>
                        </div>
                        {emp.nearest_pending_date && (
                          <p className="text-xs text-gray-500 flex items-center gap-1">
                            <Calendar size={12} /> Nearest pending: {fmtD(emp.nearest_pending_date)}
                          </p>
                        )}
                      </div>
                    ))}
                      ...
                      <tbody>
                        {drillRequests.slice(0, drillVisible).map((req) => (
                          <tr
                            key={(req as any).uniq_id ?? req.id}
                            className={`border-b border-gray-100 transition-colors ${
                              isUrgent(req) ? 'bg-red-50 hover:bg-red-100 border-l-4 border-red-400' : 'hover:bg-blue-50'
                            }`}
                          >
                            <td className="py-2 px-3 font-medium text-gray-800">{(req as any).emp_fullname ?? req.employee_name}</td>
                            <td className="py-2 px-3 hidden sm:table-cell"><span className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-medium">{(req as any).emp_dept ?? req.department}</span></td>
                            <td className="py-2 px-3 text-gray-500 hidden md:table-cell">{fmtDT((req as any).encode_date ?? req.created_at)}</td>
                            <td className="py-2 px-3 text-gray-500 hidden md:table-cell">{fmtD((req as any).effective_date)}</td>
                            <td className="py-2 px-3 text-gray-700">{(req as any).rqst_type ?? req.type}</td>
                            <td className="py-2 px-3"><span className={`px-1.5 py-0.5 rounded text-xs font-semibold capitalize ${statusBadge(getDisplayStatus(req))}`}>{getDisplayStatus(req)}</span></td>
                            <td className="py-2 px-3 text-center">
      ...
      {viewRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto z-10">
            ...
            {(() => {
              const req = viewRequest as any;
              const myStatus = req.sup_status ?? 'Pending';
